...
\subsection{Dataset}
A dataset is loaded via HuggingFace from \texttt{proteinea/secondary\_structure\_prediction} \citet{proteinea_ssp}. This dataset contains 10,792 protein sequences with per-residue annotations across five columns: the amino acid sequence (\texttt{input}), three-class secondary structure labels (\texttt{dssp3: H, E, C}), eight-class secondary structure labels (\texttt{dssp8}), per-residue disorder scores (\texttt{disorder}), and a masking column (\texttt{cb513\_mask}). The disorder column stores space-separated float values (\texttt{0.0} or \texttt{1.0}, with some positions marked as ambiguous), and the \texttt{cb513\_mask} column identifies valid sequence positions, as the raw disorder and mask strings are longer than the sequence itself due to formatting. In this work, \texttt{dssp3} is used for secondary structure and disorder is used for intrinsic disorder; input is used as an index for each sequence; \texttt{cb513\_mask} is used as well for preprocessing.
...
A single linear layer is trained with Adam (\texttt{lr=1e-3}), cross-entropy loss, full-batch gradient descent, and 50 epochs for ESM-2 probes. The two tasks differ in output dimension: the secondary structure probe maps 480 $\rightarrow$ 3 and the disorder probe maps 480 $\rightarrow$ 2. Additionally, the disorder task suffers from significant class imbalance, with disordered residues outnumbering ordered residues at approximately 20:1 in the training set; to address this, the disorder probe is trained with inverse-frequency class weighting to prevent majority-class collapse, while the secondary structure probe is trained without weighting. 
...
For the secondary structure probe on the baseline, we find that macro F1 = 0.359 and AUC=0.611. Notably, the baseline assigns F1=0.000 to strand entirely, showing that one-hot sequence composition carries no discriminative signal for beta-strands whatsoever.
...
Overall, ESM-2 embeddings improve macro F1 by +0.209 and AUC by +0.256 over baseline. The improvement is real but smaller than for secondary structure, and potentially confounded by imperfect baseline convergence under class-weighted loss.
...
\begin{figure}[H]
    ...
    \caption{ESM-2 probe accuracy vs protein length for secondary structure (left) and intrinsic disorder (right). Each point represents one test protein; the navy line shows the binned mean and the shaded region shows $\pm$ standard deviation.}
\end{figure}
